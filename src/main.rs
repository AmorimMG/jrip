...
use iced::{
    widget::{button, column, row, text}, window, Element, Length::Fill, Task
};
...
let mut content = column![row![
        text(state.current_dir.to_str().unwrap_or("unknown directory")).size(32).width(Fill),
    ...
    for file in &state.current_files{
        content = content.push(text(file.0.clone()));
    }
...
fn main() -> iced::Result{
    iced::application("Jrip",update, view).theme(|_s| iced::Theme::KanagawaDragon).run()
}
